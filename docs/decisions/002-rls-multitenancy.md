...
- **Negative / tradeoffs:** two connection strings and two roles to manage. `FORCE` also applies to the owner, so the owner-run seed (Phase 9) must `set_config('memos.agent_projects', …)` before inserting or be granted scope — a real gotcha we accept and document in the seed stub. An unset GUC yields `= ANY(NULL)` → zero rows (safe default-deny) but *silently* — a forgotten per-request `set_config` looks like "no data," not "broken auth," so the gateway always sets it and a test asserts the unset case returns nothing.
